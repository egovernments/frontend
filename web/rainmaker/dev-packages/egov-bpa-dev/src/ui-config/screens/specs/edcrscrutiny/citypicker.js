...
import { showCityPicker, validateFields } from "../utils";
import get from "lodash/get";


const applyForm = (state, dispatch) => {
  const tenantId = get(
  ...
  if (isTradeDetailsValid) {
    window.location.href =
      process.env.NODE_ENV === "production"
        ? `/citizen/edcrscrutiny/ocapply?tenantId=${tenantId}`
        : process.env.REACT_APP_SELF_RUNNING === true
          ? `/egov-ui-framework/edcrscrutiny/ocapply?tenantId=${tenantId}`
          : `/edcrscrutiny/ocapply?tenantId=${tenantId}`;
  };
};
