...
class Applications extends React.Component {
  getTaskDetails = data => {
    ...
    if (connectionNo && connectionNo !== 'NA' && applicationType.includes('MODIFY')) {
      store.dispatch(
        setRoute(`/gc/search-preview?applicationNumber=${data.applicationNo}&tenantId=${data.property.tenantId}&history=true&service=${data.service}&mode=MODIFY`)
      )
    } else {
      store.dispatch(
        setRoute(`/gc/search-preview?applicationNumber=${data.applicationNo}&tenantId=${data.property.tenantId}&history=true&service=${data.service}`)
      )
    }
  }
