...
const getSewerageDetails = async (queryObject, dispatch) => {
    try {
        ...
        if (
            ...
            dispatch(setRoute(`/gc/connection-details?connectionNumber=${get(response, 'SewerageConnections[0].connectionNo', '')}&tenantId=${get(response, 'SewerageConnections[0].tenantId', '')}&service=${serviceConst.SEWERAGE}&connectionType=${get(response, 'SewerageConnections[0].connectionType', '')}`))
        }
        return response;
    } catch (error) {
        console.log(error)
...
const getWaterDetails = async (queryObject, dispatch) => {
    try {
        ...
        if (
            ...
            dispatch(setRoute(`/gc/connection-details?connectionNumber=${get(response, 'WaterConnection[0].connectionNo', '')}&tenantId=${get(response, 'WaterConnection[0].tenantId', '')}&service=${serviceConst.WATER}&connectionType=${get(response, 'WaterConnection[0].connectionType', '')}`))
        } return response;
    } catch (error) {
        console.log(error)
    }
