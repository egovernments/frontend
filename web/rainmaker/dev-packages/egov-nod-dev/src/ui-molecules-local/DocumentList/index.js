...
class DocumentList extends Component {
  ...
  componentDidMount = () => {
    ...
    if (uploadedDocuments && Object.keys(uploadedDocuments).length) {
      ...
      Object.values(uploadedDocuments).forEach((item, index) => {
        prepareFinalObject(
          `NOD[0].NodDetail.applicationDocuments[${uploadedIndex[index]}]`,
          { ...item[0] }
        );
      });
  ...
  handleDocument = async (file, fileStoreId) => {
    ...
    prepareFinalObject("NodTemp[0].uploadedDocsInRedux", {
      ...uploadedDocuments
    });
  ...
  removeDocument = remDocIndex => {
   ...
   (getQueryArg(window.location.href, "action") === "edit"||getQueryArg(window.location.href, "action") === "EDITRENEWAL" )&&
      ...
      prepareFinalObject("NodTemp[0].removedDocs", [
        ...get(preparedFinalObject, "NodTemp[0].removedDocs", []),
        {
              ...
              >
                <Grid container={true}>
                  ...
                  <Grid item={true} xs={6} sm={6} align="left">
                    <LabelContainer
                      labelName={ document.code}
                      labelKey={document.code}
                      style={documentTitle}
                    />
                    {/* {document.required && (
