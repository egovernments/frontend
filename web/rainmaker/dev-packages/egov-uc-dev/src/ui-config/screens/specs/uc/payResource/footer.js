...
export const callPGService = async (state, dispatch) => {
  ...
  try {
    ...
    if (get(goToPaymentGateway, "Transaction.txnAmount") == 0) {
      ...
      const ackUrl = `acknowledgement?status=${"success"}&consumerCode=${consumerCode}&tenantId=${tenantId}&receiptNumber=${transactionId}&businessService=${businessService}`;
...
const moveToSuccess = (dispatch, receiptNumber) => {
  ...
  const url = `${appendUrl}/uc/acknowledgement?status=${status}&consumerCode=${consumerCode}&tenantId=${tenantId}&receiptNumber=${receiptNumber}&businessService=${businessService}&purpose=${"pay"}`;
...
const moveToFailure = dispatch => {
  ...
  const appendUrl =
    process.env.REACT_APP_SELF_RUNNING === "true" ? "/egov-ui-framework" : "";
  const url = `${appendUrl}/acknowledgement?status=${status}&consumerCode=${consumerCode}&tenantId=${tenantId}&businessService=${businessService}`
