...
export const callPGService = async (state, dispatch) => {
  ...
  try {
    ...
    if (get(goToPaymentGateway, "Transaction.txnAmount") == 0) {
      ...
      dispatch(
        setRoute(
          `acknowledgement?status=${"success"}&consumerCode=${consumerCode}&tenantId=${tenantId}&receiptNumber=${transactionId}&businessService=${businessService}`
        )
      );
    } else {
...
const moveToSuccess = (dispatch, receiptNumber) => {
  ...
  dispatch(
    setRoute(
      `${appendUrl}/uc/acknowledgement?status=${status}&consumerCode=${consumerCode}&tenantId=${tenantId}&receiptNumber=${receiptNumber}&businessService=${businessService}&purpose=pay`
    )
  );
};
const moveToFailure = dispatch => {
  ...
  dispatch(
    setRoute(
      `${appendUrl}/uc/acknowledgement?status=${status}&consumerCode=${consumerCode}&tenantId=${tenantId}&businessService=${businessService}&purpose="pay"`
    )
  );
};
