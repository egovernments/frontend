...
export const getStatusKey = status => {
  switch (status) {
    ...
    case 'PENDINGAPPROVAL ':
      return { labelName: 'Pending for Approval', labelKey: 'PENDINGAPPROVAL' }
    case 'PENDINGPAYMENT':
      return { labelName: 'Pending payment', labelKey: 'PENDINGPAYMENT' }
    case 'DOCUMENTVERIFY':
      return {
...
export const getRequiredDocData = async (action, dispatch, moduleDetails) => {
  ...
  try {
    ...
    documents = [
      {
        ...
        dropdownData: [
          ...
          {
            code: 'OWNER.OWNERSHIPPROOF',
            active: false
          },
          {
            code: 'OWNER.OWNERPHOTO',
            active: false
          },
          {
            code: 'OWNER.OLDLICENCENO',
            active: false
          }
        ],
        description: 'OWNER.OWNERIDPROOF.OWNERIDPROOF_DESCRIPTION'
      }
    ]
    ...
    const reqDocuments = getRequiredDocuments(
      documents,
      moduleName,
      footerCallBackForRequiredDataModal(moduleName)
    )
    set(
      action,
