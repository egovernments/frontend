...
const FstpInbox = () => {
  ...
  const { isLoading: isVehiclesLoading, data: vehicles } =
  ...
  let filters = {
    ...
    refernceNos:
      applicationData !== undefined && searchParams?.refernceNos?.length > 0
        ? applicationData?.applicationNo || "null"
        : "",
    vehicleIds:
      vehicles !== undefined && searchParams?.registrationNumber?.length > 0
        ? vehicles?.vehicle?.[0]?.id || "null"
        : "",
    ...
    tripOwnerIds:
      dsoData !== undefined && searchParams?.name?.length > 0
        ? dsoData?.[0]?.ownerId || "null"
        : "",
