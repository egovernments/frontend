...
const Inbox = ({ parentRoute, isSearch = false, isInbox = false }) => {
  ...
  const searchParamsKey = isInbox
    ? "fsm/inbox/searchParams"
    : "fsm/search/searchParams";
  const searchParamsValue = isInbox
    ...
    : {};
  const [searchParams, setSearchParams] = Digit.Hooks.useSessionStorage(
    searchParamsKey,
    searchParamsValue
  );
  ...
  const {
    ...
    tenantId,
    {
      ...
      toDate: searchParams?.toDate
        ? new Date(searchParams?.toDate).getTime()
        : undefined,
    },
    {
      enabled: isInbox,
  ...
  const onSearch = (params = {}) => {
    if (isSearch) {
      ...
      setSearchParams(({ applicationStatus, locality, uuid }) => ({
        applicationStatus,
        locality,
        uuid,
        ...params,
      ...
      return (
        <div>
          {!isSearch && (
            <Header>
              {t("ES_COMMON_INBOX")}
              {Number(inboxTotalCount) ? (
                <p className="inbox-count">{Number(inboxTotalCount)}</p>
              ) : null}
            </Header>
          )}
          <DesktopInbox
